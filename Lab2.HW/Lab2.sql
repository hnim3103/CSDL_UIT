...
SELECT COUNT(ChuyenNganh) AS SoChuyenGia, ChuyenNganh
FROM ChuyenGia
...
SELECT CongTy.TenCongTy, COUNT(DuAn.TrangThai) AS SoLuong
FROM CongTy
JOIN DuAn ON CongTy.MaCongTy = DuAn.MaCongTy
GROUP BY CongTy.TenCongTy
HAVING COUNT(DuAn.TrangThai) > 1
ORDER BY COUNT(DuAn.TrangThai) DESC
...
SELECT TenKyNang, COUNT(ChuyenGia_KyNang.MaKyNang) AS SoLuongCG
FROM KyNang
...
HAVING COUNT(ChuyenGia_KyNang.MaKyNang) > 2
ORDER BY TenKyNang ASC
...
SELECT TenCongTy, AVG(SoNhanVien) AS TrungBinhNV
FROM CongTy
WHERE SoNhanVien >= 50 
	AND SoNhanVien <= 150
GROUP BY TenCongTy
...
SELECT HoTen, COUNT(ChuyenGia_KyNang.MaChuyenGia) AS SoLuongKN
...
SELECT TenKyNang, COUNT(ChuyenGia_KyNang.MaKyNang) AS TongSoCG
...
SELECT HoTen, AVG(CapDo) AS CapDoTB
FROM ChuyenGia, ChuyenGia_KyNang
WHERE ChuyenGia.MaChuyenGia = ChuyenGia_KyNang.MaChuyenGia
	AND ChuyenNganh = N'Phát triển phần mềm'
GROUP BY HoTen
HAVING AVG(CapDo) > 3
